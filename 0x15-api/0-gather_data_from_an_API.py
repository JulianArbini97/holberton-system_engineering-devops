...
if __name__ == "__main__":
    ...
    all_info = requests.get('https://jsonplaceholder.typicode.com/users/{}'.format(argv[1]))
    user_name = all_info.json()
    user_name = user_name.get('name')

    """ Second we see how many tasks """
    total_tasks = 0
    all_tasks_per_userId = requests.get('https://jsonplaceholder.typicode.com/todos//?userId={}'.format(argv[1]))
    ATPU_json = all_tasks_per_userId.json()
